...
class BuildWordItemSimilarity(gokart.TaskOnKart):
    ...
    *
    ...
    item_train_data_task = gokart.TaskInstanceParameter(
        description='A task which outputs `pd.DataFrame` with columns=["item_id", "text"].')
    ...
    def requires(self):
        ...
        self.item2embedding = self._calculate_item2embedding(self.scdv, self.word2embedding, item_train_data)
        self.similarity_train_data = self._calculate_similarity_train_data(click_data, word2items)
        self.similarity_model = self._train_similarity_model(self.item2embedding, self.similarity_train_data)

        # calculate similarity
        self.word2average_embedding = self._calculate_word2average_embedding(word_data, word2items, self.item2embedding)
        self.predict_item2embedding = self._calculate_item2embedding(self.scdv, self.word2embedding, item_predict_data)
        similarity = self._calculate_similarity(self.similarity_model, self.word2average_embedding,
    ...
    def _train_scdv(self, item_train_data, text_data):
        ...
        item_text_data = redshells.data.data_frame_utils.ExtractColumnAsList(
            data_task=item_train_data, column_name='text')
    ...
    def _reduce_dimension(self, word2embedding, item2embedding):
        ...
        item2embedding = redshells.app.word_item_similarity.ApplyDimensionReductionModel(
            item2embedding_task=item2embedding,
    ...
    def _calculate_item2embedding(self, scdv, word2embedding, item_train_data):
