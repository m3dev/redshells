    ...
    def __init__(self,
        ...
        with tf.variable_scope(scope_name, reuse=tf.AUTO_REUSE):
            ...
            for l in range(num_gcn_layer):
                h = tf.keras.layers.Dropout(self.input_dropout)(h)
...
class MultiresolutionGraphAttentionNetworks(object):
    def __init__(self,
        ...
        self.dataset = _Dataset(
            self.user_ids, self.item_ids, self.ratings, self.test_size, self.batch_size, user_features=self.user_features, item_features=self.item_features)
        self.graph = None
    ...
    def predict(self, user_ids: List, item_ids: List) -> np.ndarray:
        ...
        for i in tqdm(range((np.sum(valid_indices)//self.batch_size)+1)):
            batch_ind = i*self.batch_size
            feed_dict = {
                self.graph.input_dropout: 0.0,
                self.graph.input_user: np.tile(user_indices[valid_indices],2)[batch_ind:batch_ind+self.batch_size].reshape(-1, 1),
                self.graph.input_item: np.tile(item_indices[valid_indices],2)[batch_ind:batch_ind+self.batch_size].reshape(-1, 1),
                self.graph.input_adjacency_matrix: _convert_sparse_matrix_to_sparse_tensor(adjacency_matrix),
                self.graph.input_edge_size: adjacency_matrix.count_nonzero()
            }
            with self.session.as_default():
        ...
        valid_predictions = np.array(batch_predictions_arr).flatten()
    ...
    def _make_graph(self) -> MultiresolutionGraphAttentionNetworksGraph:
        return MultiresolutionGraphAttentionNetworksGraph(
            ...
            lmda=self.lmda,
            num_gcn_layer=self.num_gcn_layer)
